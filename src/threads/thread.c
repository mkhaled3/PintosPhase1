...
//Checks if the current thread no longer has the highest priority
bool worth_yield(){
...
thread_set_nice (int nice UNUSED)
{
    ...
    //If thread current no longer has the higher priority then yield
...
{
    //load_avg is assumed to be fixed_point by default as it is the value that gets multiplied by fixed_point values
    return fp_round (multiply_fp_int (load_avg, 100));
}
...
{
    return fp_round (multiply_fp_int (thread_current()->recent_cpu, 100));
}
/*******************************************************************/
...
init_thread (struct thread *t, const char *name, int priority)
{
    ...
    t->priority = priority;
    //Default value for nice until it gets reassigned using thread_set_nice
    t->nice = 0;
    /*******************************************************************/
...
/*THis function increments recent_cpu of running thread
 * THIS FUNCTION IS CALLED EVERY (1 tick)*/
...
/* function to update priority for a single thread
 * This function is only used in advanced scheduler
 * updates priority and insures that thread's priority is constraints of priority
 * THIS FUNCTION IS CALLED EVERY (4 ticks)*/
void
update_priority(struct thread *t, void *aux UNUSED)
{
    ASSERT(thread_mlfqs);
    // priority = PRI_MAX - (recent_cpu / 4) - (nice * 2)
    t->priority = fp_chop (sub_fp_int (sub_fp_fp (cast_fp (PRI_MAX), \
							 divide_fp_int (t->recent_cpu, 4)), 2 * t->nice));
    //Insure priority is within its constrains [0, 63]
...
/*function to update load_avg of the system
 * THIS FUNCTION IS CALLED EVERY (1 sec)*/

void
update_load_avg(){
    ASSERT(thread_mlfqs);
    //Ready threads = number of ready threads + running threads(if running thread is idle don't add it)
    int readyThreadsNo = list_size(&ready_list);
    if(thread_current() != idle_thread)
        readyThreadsNo++;

    //load_avg = (59 / 60) * load_avg + (1 / 60) ready_threads
    load_avg = add_fp_fp (divide_fp_int (multiply_fp_int (load_avg, 59), 60), divide_fp_int(cast_fp(readyThreadsNo), 60));
}

/*Update recent cpu for a thread
* gets called every (1 sec) */
void
update_recent_cpu(struct thread *t, void *aux UNUSED){
    ASSERT(thread_mlfqs);
    //recent_cpu = decay * recent_cpu + nice, where
    //decay = (2 * load_average )/ (2 * load_average + 1)
    t->recent_cpu = add_fp_int (multiply_fp_fp (divide_fp_fp (multiply_fp_int (load_avg, 2), \
					  add_fp_int (multiply_fp_int (load_avg, 2), 1)), t->recent_cpu), t->nice);
}


void resort_ready_threads(){
    ASSERT(thread_mlfqs);

    list_sort(&ready_list, &less_thread_priority_comp,NULL);
